/*
	Accordion
	Ash Whiting 2011
	ash@portableinteractive.com
*/
(function ($) {

	// important. This makes all the elements show in the right place initially
    var firstLoad = 1
    ...
    function ClassicAccordion(instance, options) {
        ...
        function init() {
            ...
            accordion.addClass('accordion').css({
                ...
                accordion.children().each(function (index) {
                    ...
                    for (var i = 0; i < $(this).children().length; i++) {
                        ...
                        if ($(data).is('a')) {
                            ...
                            if ($(data).attr('target')) panel.properties.linkTarget = $(data).attr('target');
                        } else if ($(data).is('img')) {
                    ...
                    for (var i = 0; i < panelProps.length; i++) {
                        ...
                        if (self.settings.panelProperties) if (self.settings.panelProperties[index]) value = self.settings.panelProperties[index][name];

                        // if a property was not specified, take the default value
                        if (!panel.properties[name]) panel.properties[name] = value || self.settings[name];
                    }
        ...
        function loadPanels() {
            ...
            if (self.settings.shuffle) panels.sort(function () {
        ...
        function displayPanels() {
            ...
            if (self.settings.slideshow) startSlideshow();

            // pause or resume the slidethos on hover
            // also close all the panels on mouse out
            accordion.hover(function () {
                if (self.settings.slideshow && self.settings.stopSlideshowOnHover) stopSlideshow();
            },

            function () {
                if (self.settings.closePanelOnMouseOut) resetPanels();

                if (self.settings.slideshow && self.settings.stopSlideshowOnHover) startSlideshow();
            });
        }
        ...
        function createPanel(index) {
            ...
            var button = $('<div class="button"></div>').appendTo(panel);
        ...
        function openPanel(index) {
            ...
            if (currentIndex != -1) {
				...
				completeFlag = 1
				// show the button
				parseCode = panelDivs[currentIndex]
            	parseCode.find(".button").delay(800).fadeIn(400);
            }

			// hide the button
            currentIndex = index;
            parseCode = panelDivs[currentIndex]
            parseCode.find(".button").fadeOut(100);
            ...
            // check that this is the first time the page has loaded. If so, set the animation to nothing.
            if (firstLoad == 1) {
                var theInitialSetting = 0;
                firstLoad = 0
            } else {
                var theInitialSetting = self.settings.slideDuration;
            }


            openedPanel.animate(anim, {
                duration: theInitialSetting,
                complete: function () {
                    if (!flag) {
                        flag = true;
        ...
        function resetPanels() {
            ...
            var flag = false,
                ...
                // reference to the opened panel
                openedPanel = panelDivs[currentIndex],
                // the initial size of the selected opened panel (used only to calculate the progress)
                openedStartSize = parseFloat(openedPanel.css(size)),
            ...
            // if there are properties that have changed, start the animation
            openedPanel.animate(anim, {
                duration: self.settings.slideDuration,
        ...
        function showPreloader() {
            var preloader = $('<div class="preloader"></div>').hide().fadeIn(300).appendTo(accordion),
        ...
        function createCaption(captionText) {
            ...
            var panelData = panels[currentIndex],
                ...
                caption = $('<div class="caption"></div>').css({
                    'width': captionWidth,
                    'height': captionHeight,
                    'left': 0,
                    'top': 0,
                    'opacity': 0
                }).appendTo(".caption-container")

                // create the background
                captionBackground = $('<div class="caption-background"></div>').css({
                    'width': '100%',
                    'height': '100%'
                }).appendTo(caption),

                // create the content of the caption
                captionContent = $('<p></p>').html(captionText).css({
                    'width': '100%',
                    'height': '100%',
                    'opacity': 1
        ...
        function removeCaption() {
            var caption = $(".caption-container").find('.caption');
        ...
        function startSlideshow() {
            slideshowTimer = setInterval(function () {
                if (self.settings.slideshowDirection == 'next') nextPanel();
                else if (self.settings.slideshowDirection == 'previous') previousPanel();
